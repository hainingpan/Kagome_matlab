function perturbationcp(g,val,vec,vecf,enlist,parameters)
% g=0.01;
...
save(sprintf("NN%dCN%ddD%.1fg%.3f.mat",parameters.NN,parameters.cellnumber,1000*parameters.dtn,g),'-v7.3');
...
colorbar;
xlim([-4,4]);
savefig(gcf,sprintf("NN%dCN%ddD%.1fg%.3f.fig",parameters.NN,parameters.cellnumber,1000*parameters.dtn,g));
filelock=sprintf("EkdD%.1fg%.4f",1000*parameters.dtn,g);
...
axis([-5,5,-5,5]);
saveas(gcf,sprintf("EkdD%.1fg%.4f//E%.2f.png",1000*parameters.dtn,g,1000*enlist(i)));
end
filelocr=sprintf("ErdD%.1fg%.4f",1000*parameters.dtn,g);
...
title(sprintf("E=%.2f(meV)",1000*enlist(i)));
saveas(gcf,sprintf("ErdD%.1fg%.4f//E%.2f.png",1000*parameters.dtn,g,1000*enlist(i)));
end
end
